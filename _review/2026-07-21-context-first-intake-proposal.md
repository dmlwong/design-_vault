...
- **Coach and reviewer are separate roles (a firewall).** The validation rules and the gate
  tell a stakeholder what's *wrong*; they don't help them get it *right*. `brief-coach`
  (`design-brain/agents/brief-coach.md`, added 2026-07-21, draft) closes that gap — a Socratic
  writing partner that draws a strong brief *out* of the stakeholder using the same eight
  criteria, then hands it to the **independent** `brief-reviewer`. The same agent never both
  coaches and judges the same brief, or the gate passes what it authored. Lowest-infra first
  version: a Claude Project / conversation primed with the coach spec (no backend); an in-form
  "coach me" step comes once the form is hosted with a model available.
...
## Vault graph
[[design-brain/agents/brief-coach|brief-coach]] · [[design-brain/brief-contract|brief-contract]] · [[design-brain/lessons/INBOX|lessons INBOX]] · [[design-brain/orchestration|orchestration]] · [[discovery/marketiq-research-agent-n8n-upgrade|marketiq-research-agent-n8n-upgrade]] · [[discovery/research-agent-in-initiatives-cp-orbit|research-agent-in-initiatives-cp-orbit]]
<!-- graph-links:end -->
