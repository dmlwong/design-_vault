...
## Gap Report
- **Found by design-reviewer, orchestration dry run 2026-07-17, verified in source
  (`_review/2026-07-17-orchestration-dry-run.md`); fix via the normal build pipeline —
  this file is cited by recorded benchmark results, so don't silently rewrite it:**
  - *Blocker:* nested cards — the ready-state output-header `Card` wraps `MetricTiles`,
    which renders a `Card` per tile (card-in-card; violates `card-panel.md` Don't and
    AGENTS.md §4 "never nest cards").
  - *Blocker:* `InitiativePicker` row buttons all read "Select" with no row identity in
    the accessible name (violates `accessibility.md` repeated-row-action rule).
  - *Major:* async result never announced — only `generating` has `role="status"`, which
    unmounts on `ready`/`error`; the skeleton doesn't mirror the findings block or the
    supplier table; two Primary "Generate research" buttons render in the empty view
    (params bar + empty-state CTA); the `density` prop isn't propagated to the two
    tables (hardcoded `Compact`); `FaIcon` sizes are raw pixel numbers (no icon-size
    token exists — token gap).
...
## Vault graph
[[_review/2026-07-17-orchestration-dry-run|2026-07-17-orchestration-dry-run]] · [[design-brain/accessibility|accessibility]] · [[design-brain/components/card-panel|card-panel]] · [[design-brain/examples/orbit-client-marketiq-guided-workflow|orbit-client-marketiq-guided-workflow]] · [[design-brain/examples/orbit-client-marketiq-research-output-next-actions|orbit-client-marketiq-research-output-next-actions]] · [[design-brain/interaction-defaults|interaction-defaults]] · [[design-brain/patterns/config-wizard|config-wizard]] · [[design-brain/patterns/guided-conversational-workflow|guided-conversational-workflow]] · [[design-brain/patterns/review-and-approve-workflow|review-and-approve-workflow]] · [[design-brain/patterns/tool-hub|tool-hub]]
<!-- graph-links:end -->
