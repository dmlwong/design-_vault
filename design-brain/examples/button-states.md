...
- Button stories (every variant, size, state, and icon slot, plus a both-themes story):
  `packages/orbit/src/actions/Button.stories.tsx` — see `design-brain/storybook.md`
- IconButton source:
...
## Known Limits
- No built-in loading state exists, so there is no loading story either.
...
## Vault graph
[[design-brain/storybook|storybook]]
<!-- graph-links:end -->
