...
## Procedure
1. **Verify the artifacts first.** Confirm every artifact to be scored exists and is
   readable from your context *before* scoring anything. An artifact you cannot open is
   an **INACCESSIBLE — MEASUREMENT VOID** result, never a 0. (A real 2026-06-18 run
   scored both arms `0/18 FAIL` because the judge was sandboxed out of the artifact
   directory and read absence as failure — `_benchmarks/results/`.)
2. Score every rubric line independently; cite concrete evidence (file/line/screen) for
   each point given or withheld.
3. Uncertain between two scores → take the lower and note why.
4. Screen-reader claims require a real assistive-technology artifact; DOM-only evidence
   is **NEEDS HUMAN CONFIRMATION**, never a pass (vault policy — `Parked Items.md`).
5. Produce per-line scores, a total, and a 3-sentence rationale. No fix suggestions —
...
## Handoff & escalation
Results go to the owner for recording in `_benchmarks/results/`. **A passing
benchmark score is not a design review** — the 2026-07-17 dry run found 2 blockers
and 4 majors in an artifact that had blind-scored 15/16; route shipping decisions
through `design-reviewer`, not through a scorecard. Disagreement between
...
## Vault graph
[[_review/Parked Items|Parked Items]] · [[design-brain/interaction-defaults|interaction-defaults]]
<!-- graph-links:end -->
