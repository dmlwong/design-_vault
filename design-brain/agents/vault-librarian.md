...
1. **Mechanical pass** (scriptable — run the scripts, don't re-derive). Run with
   ...
   `tools/lint_frontmatter.py` (plus its `--stale-report` mode),
   `tools/gen_graph_links.py` (regenerates the Obsidian "Vault graph" footers), and
...
## Vault graph
[[AGENTS|AGENTS]] · [[design-brain/lessons/INBOX|lessons INBOX]]
<!-- graph-links:end -->
